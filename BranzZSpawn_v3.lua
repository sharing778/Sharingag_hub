...
local function findMyPlotAndBase()
    ...
    -- Fallback: usa o primeiro plot disponível
...
-- Retorna uma posição aleatória DENTRO do plot
local function getSpawnPosInsidePlot(base)
    if not base then
        return Vector3.new(0, 10, 0)
    end
    if base:IsA("Model") and base.PrimaryPart then
        local p = base.PrimaryPart
        local sx = p.Size.X * 0.35
        local sz = p.Size.Z * 0.35
    ...
    elseif base:IsA("BasePart") then
        local sx = base.Size.X * 0.35
        local sz = base.Size.Z * 0.35
